...
fn main() {
    ...
    let utxos: Vec<_> = amts
        .iter()
        .map(|&x| {
            Utxo { value: Amount::from_sat_u32(x) }
        })
        .collect();

    let bnb_selection = branch_and_bound(target, cost_of_change, Weight::MAX, fee_rate, long_term_fee_rate, &utxos);
    match bnb_selection {
        Err(SolutionNotFound) => println!("BnB found no solution as expected"),
        _ => panic!("expected no solution found"),
    }

    let srd_selection = single_random_draw(target, Weight::MAX, fee_rate, &mut thread_rng(), &utxos);
    match srd_selection {
    ...
    let select = select_coins(target, Amount::ZERO, Weight::MAX, fee_rate, long_term_fee_rate, &utxos);
    match select {
