...
fn build_lookahead<'a, T: WeightedUtxo>(
    weighted_utxos: &mut [T],
    available_value: Amount) -> Vec<Amount> {

    let mut result = Vec::new();
    result.push(weighted_utxos[0].value());
...
fn build_min_tail_weight<'a, T: WeightedUtxo>(weighted_utxos: &mut [T]) -> Vec<Weight> {
    let mut result = Vec::new();
    result.push(weighted_utxos[0].weight());
