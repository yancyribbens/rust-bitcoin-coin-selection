...
mod tests {
    ...
    fn select_coins_bnb_exhaust() {
        ...
        let (count, utxos) =
            select_coins_bnb(target, Amount::ONE_SAT, FeeRate::ZERO, FeeRate::ZERO, &pool).unwrap();

        assert!(count == ITERATION_LIMIT);
    ...
    fn select_coins_bnb_exhaust_v2() {
        ...
        let (count, utxos) = select_coins_bnb(
