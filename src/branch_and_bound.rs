...
pub fn branch_and_bound<T: Spendable>(
    ...
    let available_value: u64 = spendable_coins
        .iter()
        .filter_map(|u| crate::effective_value(fee_rate, u.total_weight(), u.value()))
        .filter_map(|sa| sa.to_unsigned().ok())
...
mod tests {
    ...
    fn select_coins_bnb_solution_proptest() {
        arbtest(|u| {
            ...
            let utxos: Vec<Utxo> = init.iter().map(|(u, _)| u).cloned().collect();
