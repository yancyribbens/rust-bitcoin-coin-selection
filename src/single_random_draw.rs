...
>(
    ...
    let result = srd_select(target, max_weight, rng, &mut utxos);
...
fn srd_select<R: rand::Rng + ?Sized>(
    ...
    max_weight: Weight,
    rng: &mut R,
    weighted_utxos: &mut [WeightedUtxo]) -> Result<(u32, Vec<usize>, bool), crate::SelectionError>  {
